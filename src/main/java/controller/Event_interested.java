    ...
    if (eventIdStr != null) {
        try {
            ...
            if (isAlreadyInterested) {
                // Nếu đã quan tâm rồi, set attribute để hiển thị nút Interested
                request.setAttribute("interestAdded", true);
            } else {
                // Nếu chưa quan tâm, thực hiện thêm quan tâm vào database
                boolean success = Event_DB.addUserInterest(userId, eventId);

                if (success) {
                    // Nếu thêm quan tâm thành công, set attribute để hiển thị nút Interested
                    request.setAttribute("interestAdded", true);
                } else {
