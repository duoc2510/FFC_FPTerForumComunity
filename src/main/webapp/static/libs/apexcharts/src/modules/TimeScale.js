    ...
    formatDates(filteredTimeScale) {
        ...
        const reformattedTimescaleArray = filteredTimeScale.map((ts) => {
            ...
            if (w.config.xaxis.labels.format === undefined) {
                ...
                if (ts.unit === 'year')
                    customFormat = dtFormatter.year
                if (ts.unit === 'month')
                    customFormat = dtFormatter.month
                if (ts.unit === 'day')
                    customFormat = dtFormatter.day
                if (ts.unit === 'hour')
                    customFormat = dtFormatter.hour
                if (ts.unit === 'minute')
                    customFormat = dtFormatter.minute
                if (ts.unit === 'second')
                    customFormat = dtFormatter.second
